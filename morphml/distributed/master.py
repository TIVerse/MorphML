...
class MasterNode:
    ...
    def __init__(self, optimizer: Any, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized MasterNode (id={self.master_id}) on {self.host}:{self.port}"
        )
    ...
    def submit_task(
        self, architecture: ModelGraph, task_id: Optional[str] = None
    ) -> str:
        """
    ...
    def _get_generation_stats(self) -> Dict[str, float]:
        ...
        if hasattr(self.optimizer, "population"):
            evaluated = [
                ind for ind in self.optimizer.population.individuals if ind.is_evaluated()
            ]
    ...
    def SubmitResult(
        ...
        try:
            if request.success:
                ...
                self.master._handle_task_result(
                    request.task_id, result, request.duration
                )
            else:
                self.master._handle_task_failure(request.task_id, request.error)
            
            return worker_pb2.ResultResponse(
                acknowledged=True, message="Result received"
            )
