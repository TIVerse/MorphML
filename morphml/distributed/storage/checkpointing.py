...
class CheckpointManager:
    ...
    def save_checkpoint(
        ...
        if self.cache:
            try:
                self.cache.set(
                    f"checkpoint:{experiment_id}:latest", checkpoint, ttl=3600
                )
                self.cache.set(
        ...
        try:
            ...
            logger.info(
                f"Saved checkpoint for {experiment_id} generation {generation}"
            )
    ...
    def load_checkpoint(
        ...
        if generation is None and self.cache:
            try:
                ...
                if checkpoint:
                    logger.info(f"Loaded checkpoint from cache (latest)")
                    return checkpoint
            except Exception as e:
        ...
        try:
            if generation is not None:
                ...
                if not self.store.exists(s3_key):
                    logger.warning(
                        f"Checkpoint not found for generation {generation}"
                    )
                    return None
                ...
                logger.info(
                    f"Loaded checkpoint for {experiment_id} generation {generation}"
                )
