...
class WorkerNode:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized WorkerNode (id={self.worker_id[:12]}) "
            f"with {self.num_gpus} GPU(s)"
        )
    ...
    def _register_with_master(self) -> None:
        ...
        logger.info(
            f"Registering with master at {self.master_host}:{self.master_port}"
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.success:
                    logger.info(
                        f"Successfully registered with master (id={response.master_id})"
                    )
                    return
                else:
    ...
    def _submit_result(
        ...
        try:
            ...
            if not response.acknowledged:
                logger.warning(
                    f"Master did not acknowledge result for task {task_id}"
                )
