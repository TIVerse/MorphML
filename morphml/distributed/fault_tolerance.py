...
class FaultToleranceManager:
    ...
    def handle_task_failure(
        self, task: Task, failure_type: FailureType, error: str
    ) -> bool:
        """
    ...
    def reassign_tasks(
        ...
        for task_id in task_ids:
            ...
            logger.info(
                f"Reassigned task {task_id} from {failed_worker_id} "
    ...
    def recover_from_checkpoint(
        self, checkpoint: Dict[str, Any], optimizer: Any
    ) -> int:
        """
        ...
        if population_data and hasattr(optimizer, "population"):
            try:
                population = [
                    Individual.from_dict(ind_dict) for ind_dict in population_data
                ]
    ...
    def _count_recent_failures(
        self, worker_id: str, window: float = 3600.0
    ) -> int:
        """
        ...
        recent = [
            f
            for f in self.worker_failures[worker_id]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_worker_failures = sum(
            len(events) for events in self.worker_failures.values()
        )
        
        circuit_breaker_states = {
            wid: breaker.get_state()
            for wid, breaker in self.circuit_breakers.items()
        }
