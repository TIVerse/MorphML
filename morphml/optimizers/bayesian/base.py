...
class BaseBayesianOptimizer:
    ...
    def __init__(self, search_space: SearchSpace, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            ...
            f"{self.n_initial_points} initial points"
        )
    ...
    def optimize(
        ...
        for iteration in range(max_evals):
            ...
            for graph in candidates:
                ...
                if self.best_individual is None or fitness > self.best_individual.fitness:
                    ...
                    logger.info(
                        f"Iteration {iteration}: New best fitness = {fitness:.4f}"
                    )
        ...
        logger.info(
            f"Optimization complete. Best fitness: {self.best_individual.fitness:.4f}"
        )
    ...
    def _encode_architecture(self, graph: ModelGraph) -> np.ndarray:
        ...
        operation_types = [
            'input', 'output', 'conv2d', 'dense', 'relu', 'sigmoid', 'tanh',
            'maxpool', 'avgpool', 'batchnorm', 'dropout', 'flatten', 'add', 'concat'
        ]
