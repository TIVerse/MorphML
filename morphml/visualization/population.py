...
class PopulationVisualizer:
    ...
    def plot_age_distribution(
        ...
        plt.ylabel('Count', fontsize=12)
        plt.title(f'Age Distribution (Mean: {sum(ages)/len(ages):.1f})', fontsize=14, fontweight='bold')
        plt.grid(True, alpha=0.3, axis='y')
        plt.tight_layout()
    ...
    def plot_population_summary(
        ...
        fitnesses = [ind.fitness for ind in population.individuals if ind.is_evaluated() and ind.fitness]
        if fitnesses:
        ...
        data = [(ind.fitness, len(ind.graph.nodes)) 
                for ind in population.individuals 
        ...
        fig.suptitle(f'Population Summary (Gen {stats.get("generation", 0)})', 
