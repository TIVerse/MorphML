...
@click.option('--export-format', '-e', type=click.Choice(['pytorch', 'keras', 'both']), default='both', help='Export format')
def run(experiment_file: str, output_dir: str, checkpoint_dir: str, resume: str, verbose: bool, export_format: str):
    """
    ...
    try:
        ...
        with Progress(
            ...
            task = progress.add_task(
                ...
                total=optimizer_config.get('num_generations', 50)
            )
...
def status(results_dir: str):
    ...
    table = Table(title=f"Experiment Status: {results_dir}", show_header=True, header_style="bold magenta")
    table.add_column("Metric", style="cyan", width=30)
...
@click.option('--format', '-f', type=click.Choice(['pytorch', 'keras', 'both']), default='pytorch', help='Export format')
    ...
    if format in ('keras', 'both') and format != 'pytorch':
        ...
        if output_keras:
            with open(output_keras, 'w') as f:
                f.write(code)
            console.print(f"[green]✓ Keras code exported to:[/green] [yellow]{output_keras}[/yellow]")
        else:
