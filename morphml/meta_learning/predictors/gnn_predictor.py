...
class GNNPredictor:
    ...
    def train(
        ...
        logger.info(
            f"Split: {len(train_dataset)} train, {len(val_dataset)} validation"
        )
    ...
    def _graph_to_pyg_data(self, graph: ModelGraph, accuracy: float) -> Data:
        ...
        if edge_list:
            ...
            edge_index = torch.tensor([[i, i] for i in range(len(node_list))], dtype=torch.long).t()
    ...
    def _encode_node(self, node, position: int, total_nodes: int) -> List[float]:
        ...
        # Operation type (one-hot)
        operation_types = [
            "input", "output", "conv2d", "conv1d", "depthwise_conv",
            "maxpool", "avgpool", "globalavgpool", "dense", "linear",
            "relu", "gelu", "sigmoid", "tanh", "batchnorm",
            "layernorm", "dropout", "residual", "concat", "add",
        ]
