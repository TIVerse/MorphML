...
class EnsemblePredictor:
    ...
    def predict(
        self, graph: ModelGraph, **predictor_kwargs
    ) -> float:
        """
        ...
        for i, predictor in enumerate(self.predictors):
            try:
                ...
                if hasattr(predictor, 'predict'):
                    ...
                    logger.warning(
                        f"Predictor {i} has no predict method, skipping"
                    )
    ...
    def batch_predict(
        self, graphs: List[ModelGraph], **predictor_kwargs
    ) -> np.ndarray:
        """
        ...
        for predictor in self.predictors:
            try:
                if hasattr(predictor, 'batch_predict'):
                    ...
                    preds = np.array([
                        predictor.predict(g, **predictor_kwargs) for g in graphs
                    ])
    ...
    def update_weights(self, new_weights: List[float]) -> None:
        ...
        if len(new_weights) != len(self.predictors):
            raise ValueError(
                f"Expected {len(self.predictors)} weights, got {len(new_weights)}"
            )
