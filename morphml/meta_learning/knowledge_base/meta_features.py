...
class MetaFeatureExtractor:
    ...
    def extract_task_features(
        self, task: TaskMetadata, normalize: bool = True
    ) -> Dict[str, float]:
        """
        ...
        if isinstance(task.input_size, (tuple, list)):
            ...
            features['input_width'] = float(task.input_size[2] if len(task.input_size) > 2 else task.input_size[1])
    ...
    def extract_architecture_features(
        ...
        op_types = [
            ...
            'relu', 'tanh', 'sigmoid', 'batchnorm',
            'dropout', 'flatten', 'input', 'output'
        ]
        ...
        features['operation_diversity'] = features['num_unique_ops'] / max(features['num_layers'], 1)
    ...
    def extract_combined_features(
        self, task: TaskMetadata, graph: ModelGraph
    ) -> Dict[str, float]:
        """
        ...
        if 'num_classes' in task_features and 'num_layers' in arch_features:
            combined['layers_per_class'] = arch_features['num_layers'] / max(task_features['num_classes'], 1)
