...
class ArchitectureTransfer:
    ...
    def transfer_architecture(
        ...
        if adaptation_strategy == "direct":
            ...
            if capacity_scale != 1.0:
                transferred = ArchitectureTransfer._scale_capacity(
                    transferred, capacity_scale
                )
    ...
    def evaluate_transferability(
        ...
        """
        if method == "simple":
            return ArchitectureTransfer._simple_transferability(
                source_task, target_task
            )
        elif method == "similarity":
            return compute_task_similarity(source_task, target_task)
        else:  # comprehensive
            return ArchitectureTransfer._comprehensive_transferability(
                source_task, target_task
            )
...
class MultiTaskNAS:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized MultiTaskNAS with {len(tasks)} tasks "
