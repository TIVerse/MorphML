...
class TestArchitectureTransfer:
    ...
    def test_direct_transfer_strategy(
        self, sample_architecture, cifar10_task, cifar100_task
    ):
    ...
    def test_evaluate_transferability_same_dataset(self, cifar10_task):
        ...
        score = ArchitectureTransfer.evaluate_transferability(
            cifar10_task, cifar10_task
        )
        
        assert score == 1.0  # Perfect transfer
    
    def test_evaluate_transferability_similar_tasks(
        self, cifar10_task, cifar100_task
    ):
        """Test transferability score for similar tasks."""
        score = ArchitectureTransfer.evaluate_transferability(
            cifar10_task, cifar100_task
        )
        
        # Should be high (same domain, input size)
        assert 0.7 <= score <= 1.0
    
    def test_evaluate_transferability_different_domains(
        self, cifar10_task, mnist_task
    ):
        """Test transferability score for different domains."""
        score = ArchitectureTransfer.evaluate_transferability(
            cifar10_task, mnist_task
        )
        
        # Should be moderate (different input, same problem type)
        assert 0.3 <= score <= 0.8
    
    def test_recommend_transfer_strategy_similar(
        self, cifar10_task, cifar100_task
    ):
        """Test strategy recommendation for similar tasks."""
        rec = ArchitectureTransfer.recommend_transfer_strategy(
            cifar10_task, cifar100_task
        )
    ...
    def test_recommend_transfer_strategy_distant(
        self, cifar10_task, mnist_task
    ):
...
class TestMultiTaskNAS:
    ...
    def test_evaluate_multi_task_fitness(
        self, sample_architecture, cifar10_task, cifar100_task
    ):
        ...
        results = multi_nas.evaluate_multi_task_fitness(
            sample_architecture, mock_evaluator
        )
    ...
    def test_create_multi_task_evaluator(
        self, sample_architecture, cifar10_task, cifar100_task
    ):
...
class TestEdgeCases:
    ...
    def test_invalid_adaptation_strategy(
        self, sample_architecture, cifar10_task, cifar100_task
    ):
