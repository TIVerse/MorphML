...
class TestMasterWorkerCommunication:
    ...
    def test_worker_registration(self):
        ...
        # Start master
        master = DistributedMaster(host="localhost", port=50051, max_workers=5)
        master.start()

        try:
            # Start worker
            worker = DistributedWorker(
                worker_id="test-worker-1", master_host="localhost", master_port=50051
            )
    ...
    def test_task_distribution(self, search_space: SearchSpace, evaluator: callable):
        ...
        # Start master
        master = DistributedMaster(host="localhost", port=50052, max_workers=2)
        master.start()
    ...
    def test_worker_failure_recovery(self, search_space: SearchSpace, evaluator: callable):
        ...
        master = DistributedMaster(host="localhost", port=50053, max_workers=3)
        master.start()
    ...
    def test_heartbeat_monitoring(self):
        ...
        master = DistributedMaster(host="localhost", port=50054, heartbeat_timeout=5)
        master.start()
...
class TestSchedulingStrategies:
    ...
    def test_load_balancing_scheduler(self, search_space: SearchSpace, evaluator: callable):
        ...
        master = DistributedMaster(host="localhost", port=50055, scheduler_type="load_balancing")
        master.start()
...
class TestFaultTolerance:
    ...
    def test_checkpoint_recovery(self, search_space: SearchSpace, evaluator: callable):
        ...
        master1 = DistributedMaster(
            host="localhost", port=50056, checkpoint_path=checkpoint_path, checkpoint_interval=5
        )
        master1.start()
        ...
        master2 = DistributedMaster(
            host="localhost", port=50056, checkpoint_path=checkpoint_path, recover=True
        )
        master2.start()
...
class TestPerformance:
    ...
    def test_throughput(self, search_space: SearchSpace, evaluator: callable):
        ...
        master = DistributedMaster(host="localhost", port=50057, max_workers=10)
        master.start()
